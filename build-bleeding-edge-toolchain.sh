...
midname="pic32mz" # pic32mx/mz
target="mipsel-$midname-elf"
installNative="$target"

binutilsVersion="2.37"
expatVersion="2.4.1"
gccVersion="11.2.0" # "10.3.0"
gdbVersion="11.1"
gmpVersion="6.2.1"
islVersion="0.23"
...
opt="-Os -EL -fsee -fomit-frame-pointer -mno-long-calls -mabi=32 -mno-shared -mno-plt"
target_gcc_opts="--enable-sjlj-exceptions --enable-sgxx-sde-multilibs --enable-static --enable-sim --disable-wchar_t --disable-tm-clone-registry --with-endian=little --enable-clocale=generic --disable-largefile --disable-libstdcxx-filesystem-ts --disable-libstdcxx-time"
case "$midname" in
  pic32mx)
    opt="$opt -march=m4k -msoft-float -mips32r2 -mips16 -minterlink-mips16"
    target_gcc_opts="$target_gcc_opts --with-arch=m4k --with-float=soft"
    ;;
  pic32mz)
    opt="$opt -march=m14kc -mmicromips"
    target_gcc_opts="$target_gcc_opts --with-arch=m14kc --with-fp-32=64"
    ;;
esac
...
BASE_CXXFLAGS_FOR_TARGET="-pipe -ffunction-sections -fdata-sections -fexceptions"
...
copyNanoLibraries() {
	...
	for multilib in ${multilibs}; do
		...
		if [ -f "${sourceDirectory}/librdimon.a" ]; then cp "${sourceDirectory}/librdimon.a" "${destinationDirectory}/librdimon_nano.a"; fi
...
download "${islArchive}" "http://sources.buildroot.net/isl/${islArchive}"
...
case "$midname" in
  pic32mx)
    (
      cd ${sources}/${gcc} &&
      (patch -p1 <<EOF
diff -r -u gcc-11.2.0/gcc/config/mips/elf.h gcc-11.2.0.00/gcc/config/mips/elf.h
--- gcc-11.2.0/gcc/config/mips/elf.h    2021-07-28 08:55:07.096284678 +0200
+++ gcc-11.2.0.00/gcc/config/mips/elf.h 2021-12-25 17:17:35.326177189 +0100
@@ -46,3 +46,8 @@
...
+
diff -r -u gcc-11.2.0/gcc/config/mips/t-elf gcc-11.2.0.00/gcc/config/mips/t-elf
--- gcc-11.2.0/gcc/config/mips/t-elf    2021-07-28 08:55:07.104284790 +0200
+++ gcc-11.2.0.00/gcc/config/mips/t-elf 2021-12-25 17:17:55.093685956 +0100
@@ -18,6 +18,6 @@
...
EOF
    ...
    ;;
  pic32mz)
    cat >${sources}/${gcc}/gcc/config/mips/t-elf <<EOF
...
buildGccFinal "-final" "$opt" "${installNative}" "${documentationTypes}"
...
buildMingw() {
	...
	flags="$opt -pipe -Wp,-D_FORTIFY_SOURCE=2 -fno-plt -fstack-protector -fexceptions --param=ssp-buffer-size=4"
	...
	export LDFLAGS="-Wl,$opt,--sort-common,--as-needed -fstack-protector -lssp ${LDFLAGS-}"
